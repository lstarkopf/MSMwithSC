...
autoplot.MSMwithSC <- function(object,estimator,...){
    ...
    estimator <- match.arg(estimator, choices =  names(object)[-length(object)], several.ok = TRUE)
    name.treatment <- eval(object$call[["treatment"]])
    dd = object[[estimator]]
    nn = length(dd$surv.yes)
    dd = data.frame(
        Exposure = rep(eval(object$call$grid),2),
        Treatment = rep(c("Treated","Untreated"),rep(nn,2)),
                    SurvivalProbability = c(dd$surv.yes,dd$surv.no))
    g = ggplot(dd,aes(Exposure,SurvivalProbability,color = Treatment,group = Treatment))+geom_line()
    g+ylim(c(0,1))
}
